...
export default async function TileDetailsPage({ params }) {
  ...
  try {
    const res = await fetch(process.env.API_URL + "/tiles/" + tileId, { cache: "no-store" });
    if (res.ok) {
  ...
  if (tileData === null) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 mt-2">It might still be loading — please try again in a moment.</p>
